...
class Local_Model(nn.Module):
    def __init__(self, **params):
        ...
        self.inter_learn_layer1 = nn.Linear((self.disen_emb_dim+self.embed_id_dim), self.disen_emb_dim).to(self.device)
        ...
        self.batch_norm2 = nn.BatchNorm1d(int(self.disen_emb_dim//2)).to(self.device)
        self.inter_learn_layer3 = nn.Linear(int(self.disen_emb_dim//2), int(self.disen_emb_dim // 4)).to(self.device)
        nn.init.xavier_uniform_(self.inter_learn_layer3.weight)
        self.batch_norm3 = nn.BatchNorm1d(int(self.disen_emb_dim // 4)).to(self.device)
        self.classifier_layer = nn.Linear(int(self.disen_emb_dim // 4), 1).to(self.device)
    ...
    def forward(self, nodes_u, nodes_v):
        ...
        return u_common_feats,u_specific_feats, v_feats, pred_prob.squeeze()
    ...
    def orthogonal_loss(self,u_common_feats,u_specific_feats):
        return torch.sum((u_common_feats*u_specific_feats).sum(dim=1)**2)
